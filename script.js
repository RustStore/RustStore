...
function setupNavigation() {
    ...
    // Hide Raiding Kits section by default
    if (raidingKitsSection) {
        raidingKitsSection.style.display = 'none';
        raidingKitsSection.style.opacity = '0';
        raidingKitsSection.style.transform = 'translateY(20px)';
    }
        ...
        button.addEventListener('click', (e) => {
            ...
            if (buttonText === 'vip') {
                ...
                if (currentSection === resourceKitsSection) {
                    ...
                    showSection(resourceKitsSection, [storeSection, vipSection, raidingKitsSection]);
                    currentSection = resourceKitsSection;
                }
            } else if (buttonText === 'raiding kits') {
                console.log('=== RAIDING KITS CLICKED ===');
                console.log('Button text matched:', buttonText);
                // If Raiding Kits is already shown, hide it
                if (currentSection === raidingKitsSection) {
                    hideSection(raidingKitsSection);
                    currentSection = null;
                } else {
                    console.log('Showing raidingKitsSection:', raidingKitsSection);
                    showSection(raidingKitsSection, [storeSection, vipSection, resourceKitsSection]);
                    currentSection = raidingKitsSection;
                }
            } else {
                ...
                if (currentSection === storeSection) {
                    ...
                    showSection(storeSection, [vipSection, resourceKitsSection, raidingKitsSection]);
...
function createResourceKitsSection() {
    ...
    const resources = [
        {
            ...
            resource: 'wood',
            amount: '20,000 Wood',
            image: './photos resource kits/Wood.png' // Wood image
        },
        {
            name: 'STONE PACK',
            ...
            resource: 'stone',
            amount: '30,000 Stone',
            image: './photos resource kits/Stone.png' // Stone image
        },
        {
            ...
            image: './photos resource kits/Metal Fragments.png' // Metal fragments image
        },
        {
            ...
            image: './photos resource kits/HQM.png' // HQM image
        },
        {
            name: 'CLOTH BUNDLE',
            icon: 'fas fa-tshirt',
            ...
            resource: 'cloth',
            amount: '2,500 Cloth',
            image: './photos resource kits/Cloth.png' // Cloth image
        },
        {
            name: 'LOW GRADE FUEL',
            icon: 'fas fa-fire',
            ...
            image: './photos resource kits/Low Grade Fuel.png' // Low grade fuel image
        },
        {
            name: 'SCRAP',
            icon: 'fas fa-recycle',
            ...
            resource: 'scrap',
            amount: '3,000 Scrap',
            image: './photos resource kits/Scrap.png' // Scrap image
        }
    ];
    ...
    resources.forEach(resource => {
        ...
        if (resource.image) {
            ...
            // Use a default background for scrap since no photo is available
            imageBg.style.cssText = `
                position: absolute; 
                top: 0; 
                left: 0; 
        ...
        contentSection.style.cssText = 'padding: 2rem; flex-grow: 1; display: flex; flex-direction: column;';
        ...
        button.innerHTML = `<i class="fas fa-shopping-cart" style="margin-right: 0.8rem;"></i>Purchase ${resource.name}`;
...
// Create Raiding Kits Section with rotating photos
function createRaidingKitsSection() {
    const raidingKitsSection = document.createElement('div');
    raidingKitsSection.className = 'raiding-kits-section';
    raidingKitsSection.id = 'raidingKitsSection';
    raidingKitsSection.style.cssText = `
        position: relative; 
        z-index: 10; 
        padding: 120px 20px 120px 20px; 
        min-height: calc(100vh - 240px); 
    ...
    const raidingKits = [
        {
            ...
            price: '$10.99',
            priceValue: 1099,
            items: ['12 Satchels', '256 Explosive Rounds'],
            images: [
                './photos raiding kits/Satchel-Charge.png',
                './photos raiding kits/Explosive-Rounds.png'
            ]
        },
        {
            ...
            price: '$11.99',
            priceValue: 1199,
            items: ['Rocket Launcher', '9 Rockets'],
            images: [
                './photos raiding kits/Rocket-Launcher.png',
                './photos raiding kits/Rocket-Ammo.png'
            ]
        },
        {
            ...
            price: '$16.99',
            priceValue: 1699,
            items: ['2 Rocket Launchers', '18 Rockets', '6 High Velocity Rockets'],
            images: [
                './photos raiding kits/Rocket-Launcher.png',
                './photos raiding kits/Rocket-Ammo.png'
            ]
        ...
        },
        {
            ...
            price: '$13.99',
            priceValue: 1399,
            items: ['10 C4\'s', '384 Explosive Rounds'],
            images: [
                './photos raiding kits/C4.png',
                './photos raiding kits/Explosive-Rounds.png'
            ]
        ...
        {
            name: 'EXPLOSIVE ADDICTED',
            icon: 'fas fa-fire',
            color: '#FF0000',
            secondaryColor: '#DC143C',
            price: '$24.99',
            priceValue: 2499,
            items: ['3 Rocket Launchers', '27 Rockets', '9 High Velocity Rockets', '256 Explosive Rounds', '8 C4\'s'],
            images: [
                './photos raiding kits/Rocket-Launcher.png',
                './photos raiding kits/Rocket-Ammo.png',
                './photos raiding kits/Explosive-Rounds.png',
    ...
    const container = document.createElement('div');
    container.className = 'raiding-kits-container';
    container.style.cssText = 'max-width: 1200px; margin: 0 auto;';

    // Header
    const header = document.createElement('div');
    header.className = 'raiding-kits-header';
    header.style.cssText = 'text-align: center; margin-bottom: 3rem;';

    const title = document.createElement('h2');
    title.className = 'raiding-kits-title';
    title.style.cssText = 'font-size: 3rem; font-weight: 700; color: #ff6b35; margin-bottom: 1rem; text-shadow: 0 0 20px rgba(255, 107, 53, 0.5);';
    title.innerHTML = '<i class="fas fa-bomb" style="margin-right: 1rem;"></i>RAIDING KITS';

    const subtitle = document.createElement('p');
    subtitle.className = 'raiding-kits-subtitle';
    subtitle.style.cssText = 'font-size: 1.2rem; color: #fff; max-width: 600px; margin: 0 auto; text-shadow: 0 0 10px rgba(0, 0, 0, 0.8);';
    subtitle.textContent = 'Powerful raiding equipment to dominate your enemies';
    ...
    // Grid
    const grid = document.createElement('div');
    grid.className = 'raiding-kits-grid';
    grid.style.cssText = 'display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 2rem; margin-bottom: 3rem;';

    raidingKits.forEach((kit, kitIndex) => {
        const card = document.createElement('div');
        card.className = 'raiding-kit-card';
        card.style.cssText = `
            background: rgba(0, 0, 0, 0.9); 
            ...
            text-align: center; 
            backdrop-filter: blur(15px); 
            box-shadow: 0 15px 40px ${kit.color}40; 
            overflow: hidden; 
            transition: all 0.3s ease;
        `;
        ...
        const imageDiv = document.createElement('div');
        imageDiv.className = 'raiding-kit-image';
        imageDiv.style.cssText = `
            height: 250px; 
        ...
        // Create rotating background images
        kit.images.forEach((image, imageIndex) => {
            const imageBg = document.createElement('div');
            imageBg.className = 'rotating-image';
            imageBg.style.cssText = `
                position: absolute; 
                top: 0; 
                left: 0; 
        ...
        const name = document.createElement('h3');
        name.className = 'raiding-kit-name';
        name.style.cssText = 'font-size: 1.8rem; color: #fff; margin: 0; font-weight: 700; text-shadow: 0 0 20px rgba(0, 0, 0, 0.8);';
        ...
        const price = document.createElement('div');
        price.className = 'raiding-kit-price';
        price.style.cssText = `font-size: 2.5rem; color: ${kit.color}; margin-bottom: 1.5rem; font-weight: 700;`;
        price.textContent = kit.price;

        const details = document.createElement('div');
        details.className = 'raiding-kit-details';
        details.style.cssText = `
            background: ${kit.color}1A; 
            border-radius: 15px; 
            padding: 1.5rem; 
            margin-bottom: 2rem; 
            border: 1px solid ${kit.color}4D;
        `;

        const detailsTitle = document.createElement('h4');
        detailsTitle.style.cssText = `color: ${kit.color}; font-size: 1.3rem; margin-bottom: 1rem; font-weight: 600;`;
        detailsTitle.innerHTML = `<i class="${kit.icon}" style="margin-right: 0.5rem;"></i>Kit Contents:`;
        ...
        kit.items.forEach(item => {
            const listItem = document.createElement('li');
            listItem.style.cssText = `padding: 0.8rem 0; color: #fff; border-bottom: 1px solid ${kit.color}33;`;
            listItem.innerHTML = `<i class="fas fa-check" style="color: ${kit.color}; margin-right: 0.8rem;"></i>${item}`;
            list.appendChild(listItem);
        });
        ...
        const button = document.createElement('button');
        button.className = 'raiding-kit-button';
        button.style.cssText = `
            background: linear-gradient(45deg, ${kit.color}, ${kit.secondaryColor}); 
            border: none; 
            color: #fff; 
            padding: 1.2rem 2rem; 
            border-radius: 30px; 
            ...
            margin-top: auto;
        `;
        button.setAttribute('data-kit', kit.name.toLowerCase().replace(/\s+/g, '_'));
        button.setAttribute('data-price', kit.priceValue);
        button.innerHTML = `<i class="fas fa-shopping-cart" style="margin-right: 0.8rem;"></i>Purchase ${kit.name}`;

        // Add click event for raiding kit purchase
        button.addEventListener('click', function() {
            const kitType = this.getAttribute('data-kit');
            const price = this.getAttribute('data-price');
            purchaseRaidingKit(kitType, price, this);
        });
        ...
        startImageRotation(imageDiv, kit.images);
    });

    container.appendChild(header);
    container.appendChild(grid);
    raidingKitsSection.appendChild(container);

    // Insert before the bottom bar
    const bottomBar = document.querySelector('.bottom-bar');
    bottomBar.parentNode.insertBefore(raidingKitsSection, bottomBar);

    return raidingKitsSection;
}

// Function to start image rotation
function startImageRotation(imageContainer, images) {
...
async function purchaseRaidingKit(kitType, price, button) {
    console.log('=== PURCHASING RAIDING KIT ===');
    ...
    try {
        const response = await apiRequest('/api/purchase-raiding-kit', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (response.success) {
            showSuccess(`Successfully purchased ${kitType} kit!`);
            // Update cart if needed
            await loadCart();
        } else {
            showError(response.message || 'Failed to purchase raiding kit');
        }
    } catch (error) {
        console.error('Error purchasing raiding kit:', error);
        showError('Failed to purchase raiding kit. Please try again.');
    } finally {
        button.innerHTML = originalText;
        button.disabled = false;
    }
...
async function purchaseResource(resourceType, price, button) {
    ...
    // Get user ID (Gamertag/PSN ID)
    const userId = prompt('Please enter your Gamertag or PSN ID:');
    if (!userId) {
        showError('User ID is required for purchase.');
        return;
    }
...
// Initialize Raiding Kits section
console.log('=== CREATING RAIDING KITS SECTION ===');
const raidingKitsSection = createRaidingKitsSection();
console.log('Raiding Kits section created:', raidingKitsSection);

// PayPal Checkout Integration
function initializePayPalCheckout() {
    // PayPal Checkout script will be loaded
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        ...
                        description: currentPurchase.itemName
                    }]
                });
            },
...
// Global variable to track current purchase
...
function handleSuccessfulPayment(details) {
    ...
    // Store purchase in localStorage (for demo - in production, use a database)
    const purchase = {
        ...currentPurchase,
    ...
    const purchases = JSON.parse(localStorage.getItem('rustStorePurchases') || '[]');
    purchases.push(purchase);
    localStorage.setItem('rustStorePurchases', JSON.stringify(purchases));
...
function showPaymentSuccess(purchase) {
    ...
    // Get the predetermined message for this kit
    const messageMap = getMessageMap();
    const message = messageMap[purchase.itemType] || 'i need wood';

    modalContent.innerHTML = `
        <div style="margin-bottom: 2rem;">
            <i class="fas fa-check-circle" style="font-size: 4rem; color: #00ff00; margin-bottom: 1rem;"></i>
            <h3 style="color: #00ff00; font-size: 1.8rem; margin-bottom: 1rem;">Payment Successful!</h3>
            <p style="color: #fff; font-size: 1.1rem; margin-bottom: 0.5rem;">${purchase.itemName}</p>
            <p style="color: #00ff00; font-size: 1.2rem; font-weight: 700;">$${purchase.price}</p>
        </div>
        
        <div style="background: rgba(0, 255, 0, 0.1); border-radius: 15px; padding: 1.5rem; margin-bottom: 2rem; border: 1px solid rgba(0, 255, 0, 0.3);">
            <h4 style="color: #00ff00; font-size: 1.2rem; margin-bottom: 1rem;">How to Claim Your Kit:</h4>
            <p style="color: #fff; margin-bottom: 0.5rem;">1. Join the Rust server</p>
            <p style="color: #fff; margin-bottom: 0.5rem;">2. Use the predetermined message:</p>
            <p style="color: #00ff00; font-weight: 700; font-size: 1.1rem; margin-bottom: 1rem;">"${message}"</p>
            <p style="color: #fff; margin-bottom: 0.5rem;">3. Your kit will be delivered automatically!</p>
        </div>
...
// Get the mapping of kit types to predetermined messages
function getMessageMap() {
    return {
        ...
        // Raiding Kits
        'beginners_raiding_kit': 'i need wood',
        'rookies_rocket_kit': 'i need scrap',
        'rocket_mania_kit': 'i need scrap',
        'demolitioner_kit': 'i need scrap',
        'base_breacher': 'i need scrap',
        'explosive_addicted': 'i need scrap'
    };
}
...
// Pay with Cash App
function payWithCashApp(itemType, price, itemName) {
    const priceInDollars = (price / 100).toFixed(2);
    
    // Show Cash App instructions
    showPaymentInstructions('Cash App', itemName, priceInDollars);
}

// Show payment instructions
function showPaymentInstructions(method, itemName, price) {
    closePaymentModal();
    ...
    if (method === 'Cash App') {
        instructions = `
            <p style="color: #fff; margin-bottom: 1rem;">1. Open Cash App on your phone</p>
            <p style="color: #fff; margin-bottom: 1rem;">2. Send $${price} to: <strong style="color: #00d632;">$YourRustStore</strong></p>
            <p style="color: #fff; margin-bottom: 1rem;">3. Include "${itemName}" in the payment note</p>
            <p style="color: #fff; margin-bottom: 1rem;">4. Contact an admin in Discord with your payment proof</p>
            <p style="color: #fff; margin-bottom: 1rem;">5. Admin will activate your kit for in-game claiming</p>
        `;
    }
    ...
    modalContent.innerHTML = `
        <h3 style="color: #ffd700; font-size: 1.8rem; margin-bottom: 1.5rem;">
            <i class="fas fa-info-circle" style="margin-right: 0.5rem;"></i>Payment Instructions
        </h3>
        ${instructions}
        <button onclick="closeInstructionsModal()" style="
